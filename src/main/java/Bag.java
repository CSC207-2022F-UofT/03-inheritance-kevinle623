...
public abstract class Bag {
    ...
    public String popItem(){
        if (this.numberOfContents == 0){
            ...
            String popitem = this.contents[this.numberOfContents - 1];

            this.numberOfContents = this.numberOfContents - 1;
            String[] newList = new String[this.numberOfContents];
            int counter_so_far = 0;
            for (String x: this.contents){
                if (counter_so_far < this.numberOfContents) {
                    newList[counter_so_far] = x;
                    counter_so_far ++;
                }
    ...
    public void increaseCapacity(int n) {
        ...
        String[] newContent = new String[this.capacity];
        int index_so_far = 0;
        for (String x: this.contents){
            newContent[index_so_far] = x;
            index_so_far ++;
